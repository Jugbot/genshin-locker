...
import { createWorker, createScheduler, OEM, PSM, ImageLike, RecognizeOptions } from "tesseract.js";
...
export class OCR {
  ...
  async recognize(image: ImageLike, options?: Partial<RecognizeOptions>) {
    return this.scheduler.addJob("recognize", image, options).then(({data}) => data.text)
  }
}